...
onAuthStateChanged(auth, async (user) => {
  ...
  currentUser = user;
  userData = { email: user.email, name: user.displayName || 'User', plan: 'free' };
  try { await loadUserProfile(); } catch (err) {
    showToast('Database access blocked. Please set Firebase rules.', 'error', 6000);
  }
  setupUI();
});
...
async function loadUserProfile() {
  ...
  else await set(ref(db, 'users/' + currentUser.uid), userData);
}
...
function startLinksListener() {
  if (linksListener && linksListener.off) linksListener.off();
  linksListener = onValue(ref(db, 'links'), (snap) => {
    allLinks = [];
    snap.forEach((child) => {
