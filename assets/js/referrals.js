...
async function loadData() {
  ...
  // Load referred users (users who have this user's referralCode or username)
  const refUsername = (userData.username || '').toLowerCase();
  const usersSnap = await get(ref(db, 'users'));
  usersSnap.forEach((child) => {
    const u = child.val();
    if (u.referredBy === refUsername || u.referredBy === userData.referralCode || u.referredBy === currentUser.uid) {
      referredUsers.push({
...
function renderAll() {
  ...
  animateCounter(document.getElementById('statRefEarnings'), totalCommission, { prefix: '$', decimals: 2 });
...
function renderReferredTable() {
  ...
  tbody.innerHTML = referredUsers.map((u, idx) => `
    <tr>
      <td>
        <div class="link-dest">
          <div class="favicon" id="refav_${idx}"></div>
          <div>
  ...
  referredUsers.forEach((u, idx) => {
    const el = document.getElementById('refav_' + idx);
